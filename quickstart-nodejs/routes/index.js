...
router.get('/subscriptionSucces', function(req, res, next){
  conn.query(`UPDATE FriendlyTextAccounts SET activeSubscription = 1, subscribed = (curdate() + INTERVAL 1 MONTH) WHERE email = "${req.cookies["userData"]["username"]}";`, 
  function (err, results, fields) {
    if (err) throw err;
    else {
      res.render('index', { title: 'FriendlyText' });
...
router.get('/unsubscribe', function(req, res, next){
  conn.query(`UPDATE FriendlyTextAccounts SET activeSubscription = 0 WHERE email="${req.cookies["userData"]["username"]}";`, 
  function (err, results, fields) {
    if (err) throw err;
    else {
      res.render('index', { title: 'FriendlyText' });
...
router.get('/GetTokenAndSubdomain', function(req, res) {
  ...
  conn.query(`SELECT email FROM FriendlyTextAccounts WHERE email="${req.cookies["userData"]["username"]}" AND (uses < 5 OR subscribed >= curdate())`, 
  function (err, results, fields) {
    if (err) throw err;
    else
      if(results.length == 1){
        conn.query(`UPDATE FriendlyTextAccounts SET uses = uses + 1 WHERE email = "${req.cookies["userData"]["username"]}";`, 
        function (err, results, fields) {
          if (err) throw err;
        });
      try {
      ...
      resp.data["flaggedTokens"].forEach(elem => {
        ...
        if(elem["suggestions"][0]["suggestion"].match(/[ ,.!?()]/g) == null)
          for(var i = elem["offset"] + globalOffset; i <= elem["offset"] + elem["suggestions"][0]["suggestion"].length + globalOffset; ++i){
            if( i >= text.length || text[i].match(/[ \,\.\!\?\(\)]/g) != null){
